...
public final class EditGroupPage extends KnownPage {
    ...
    @com.angkorteam.pluggable.widget.LabelField(label = "Name", order = 1)
    private String name;
    ...
    private com.angkorteam.pluggable.entity.Role[] roles;
    ...
    public EditGroupPage(PageParameters parameters) {
        ...
        ddl.append("select role.* from "
                + TableUtilities
                        .getTableName(com.angkorteam.pluggable.entity.Role.class)
                + " role ");
        ddl.append("inner join " + TableUtilities.getTableName(RoleGroup.class)
                + " role_group on role."
                + com.angkorteam.pluggable.entity.Role.ID
                + " = role_group." + RoleGroup.ROLE_ID + " ");
        ddl.append("where role_group." + RoleGroup.GROUP_ID + " = ?");
        List<com.angkorteam.pluggable.entity.Role> roles = jdbcTemplate
                .query(ddl.toString(),
                        new EntityRowMapper<com.angkorteam.pluggable.entity.Role>(
                                com.angkorteam.pluggable.entity.Role.class),
                        groupId);
        this.roles = roles
                .toArray(new com.angkorteam.pluggable.entity.Role[roles
                        .size()]);
    ...
    @com.angkorteam.pluggable.widget.Button(label = "Delete", validate = false, order = 2)
    ...
    @com.angkorteam.pluggable.widget.Button(label = "Okay", validate = true, order = 3)
    public Navigation okayClick() {
        ...
        if (roles != null && roles.length > 0) {
            ...
            for (com.angkorteam.pluggable.entity.Role role : roles) {
