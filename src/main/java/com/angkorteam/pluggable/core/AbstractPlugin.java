...
import com.angkorteam.pluggable.migration.PluginMigrator;
import com.angkorteam.pluggable.page.KnownPage;
...
import com.angkorteam.pluggable.wicket.RestController;
                ...
                for (String javaPackage : getPackages()) {
                    ...
                    for (Class<? extends Job> job : jobs) {
                        if (!Modifier.isAbstract(job.getModifiers())
                                && job.isAnnotationPresent(com.angkorteam.pluggable.quartz.Scheduled.class)) {
