...
import com.angkorteam.pluggable.entity.ApplicationSetting;
import com.angkorteam.pluggable.entity.Group;
import com.angkorteam.pluggable.entity.Job;
import com.angkorteam.pluggable.entity.PluginRegistry;
import com.angkorteam.pluggable.entity.PluginSetting;
...
import com.angkorteam.pluggable.error.AccessDeniedPage;
import com.angkorteam.pluggable.error.ExceptionErrorPage;
import com.angkorteam.pluggable.error.InternalErrorPage;
import com.angkorteam.pluggable.error.PageExpiredErrorPage;
import com.angkorteam.pluggable.migration.AbstractPluginMigrator;
import com.angkorteam.pluggable.page.GroupManagementPage;
import com.angkorteam.pluggable.page.JobManagementPage;
import com.angkorteam.pluggable.page.JsonDocPage;
import com.angkorteam.pluggable.page.JvmPage;
import com.angkorteam.pluggable.page.RoleManagementPage;
...
import com.angkorteam.pluggable.wicket.authroles.authorization.strategies.role.annotations.AuthorizeAction;
import com.angkorteam.pluggable.wicket.authroles.authorization.strategies.role.annotations.AuthorizeActions;
...
public class FrameworkUtilities {
    ...
    public static boolean hasAccess(Roles roles, Class<?> clazz) {
        ...
        if (authorizeInstantiation == null || authorizeInstantiation.roles() == null || authorizeInstantiation.roles().length == 0) {
            ...
            for (com.angkorteam.pluggable.wicket.authroles.Role role : authorizeInstantiation.roles()) {
            ...
            if (annotated != null && !annotated.isEmpty()) {
                for (Class<?> clazz : annotated) {
                    ...
                    if (authorizeAction.roles() != null && authorizeAction.roles().length > 0) {
                        for (com.angkorteam.pluggable.wicket.authroles.Role role : authorizeAction.roles()) {
            ...
            for (Class<?> controller : controllers) {
                for (Method method : ReflectionUtils.getAllMethods(controller)) {
                    ...
                    if (secured != null && secured.roles() != null && secured.roles().length > 0) {
                        for (com.angkorteam.pluggable.wicket.authroles.Role role : secured.roles()) {
                            if (!roles.containsKey(role.name())) {
                                roles.put(role.name(), role.description());
                            }
                        }
        ...
        if (authorizeActions != null && authorizeActions.actions() != null && authorizeActions.actions().length > 0) {
            for (AuthorizeAction action : authorizeActions.actions()) {
                if (action.roles() != null && action.roles().length > 0) {
                    for (com.angkorteam.pluggable.wicket.authroles.Role role : action.roles()) {
                        if (!roles.contains(role.name())) {
                            roles.add(role.name());
                        }
                    }
