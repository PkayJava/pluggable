...
public final class EditRolePage extends KnownPage {
    ...
    @com.itrustcambodia.pluggable.widget.Select2MultiChoice(label = "Groups", minimumInputLength = 1, order = 4, provider = GroupProvider.class)
    private Group[] groups;

    @com.itrustcambodia.pluggable.widget.Select2MultiChoice(label = "Groups", minimumInputLength = 1, order = 4, provider = UserProvider.class)
    ...
    @com.itrustcambodia.pluggable.widget.Button(label = "Cancel", order = 1, validate = false)
    public Navigation cancelClick() {
        return new Navigation(RoleManagementPage.class);
    }

    @com.itrustcambodia.pluggable.widget.Button(label = "Okay", order = 2, validate = true)
