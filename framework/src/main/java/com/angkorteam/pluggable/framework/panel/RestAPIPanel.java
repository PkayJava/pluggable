...
public class RestAPIPanel extends Panel {
    ...
    public RestAPIPanel(String id, List<RestAPIForm> restAPIForms) {
        ...
        WebMarkupContainer restAccordion = new WebMarkupContainer(
                "restAccordion");
        final String restAccordionId = restAccordion.getMarkupId(true);
        add(restAccordion);

        ListView<RestAPIForm> restItems = new ListView<RestAPIForm>(
                "restItems", this.restAPIForms) {
            ...
            protected void populateItem(ListItem<RestAPIForm> item) {
                ...
                head.add(AttributeModifier.replace("data-parent", "#"
                        + restAccordionId));
                ...
                ListView<RequestMethod> methods = new ListView<RequestMethod>(
                    ...
                    protected void populateItem(ListItem<RequestMethod> item) {
                        Label method = new Label("method", item
                                .getModelObject().name());
                        if (item.getModelObject() == RequestMethod.GET) {
                            method.add(AttributeModifier.replace("class",
                                    "label label-info"));
                ...
                ListView<String> roles = new ListView<String>("roles",
                        Arrays.asList(item.getModelObject().getRoles())) {
                ...
                roles.setVisible(item.getModelObject().getRoles() != null
                        && item.getModelObject().getRoles().length != 0);
                body.add(roles);
                ...
                ListView<List<Map<String, String>>> urlParameters = new ListView<List<Map<String, String>>>(
                    ...
                    protected void populateItem(
                            ListItem<List<Map<String, String>>> item) {
                        ListView<Map<String, String>> urlParameter = new ListView<Map<String, String>>(
                                "urlParameter", item.getModelObject()) {

                            private static final long serialVersionUID = 3849467973532369190L;

                            @Override
                            protected void populateItem(
                                    ListItem<Map<String, String>> item) {
                                Label name = new Label("name", item
                                        .getModelObject().get("name"));
                                item.add(name);
                ...
                urlParameters.setVisible(item.getModelObject()
                        .getUrlParameters() != null
                        && !item.getModelObject().getUrlParameters().isEmpty());
                ...
                ListView<List<Map<String, String>>> formParameters = new ListView<List<Map<String, String>>>(
                    ...
                    protected void populateItem(
                            ListItem<List<Map<String, String>>> item) {
                        ListView<Map<String, String>> formParameter = new ListView<Map<String, String>>(
                                "formParameter", item.getModelObject()) {
                ...
                ListView<Map<String, String>> headers = new ListView<Map<String, String>>(
                        "headers", item.getModelObject().getHeaders()) {
                    ...
                    protected void populateItem(
                        ...
                        Label description = new Label("description", item
                                .getModelObject().get("description"));
                        item.add(description);
                    }
                };
                headers.setVisible(item.getModelObject().getHeaders() != null
                        && !item.getModelObject().getHeaders().isEmpty());
                body.add(headers);

                ListView<Map<String, String>> errors = new ListView<Map<String, String>>(
                        "errors", item.getModelObject().getErrors()) {
                    ...
                    protected void populateItem(
                            ListItem<Map<String, String>> item) {
                        Label name = new Label("code", item.getModelObject()
                                .get("code"));
                        item.add(name);
                ...
                errors.setVisible(item.getModelObject().getErrors() != null
                        && !item.getModelObject().getErrors().isEmpty());
                body.add(errors);

                Label responseObject = new Label("responseObject", item
                        .getModelObject().getResponseObject());

                Label responseDescription = new Label("responseDescription",
                ...
                responseObject.setVisible(!"".equals(item.getModelObject()
                        .getResponseObject()));
