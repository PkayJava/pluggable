...
public class ResourceController {
    ...
    public Result file(AbstractWebApplication application,
        ...
        if (type == null || type.equals("")) {
            type = application.select(FrameworkConstants.REPOSITORY,
                    String.class);
        } else {
            if (!type.equals(FrameworkConstants.REPOSITORY_AMAZON_S3)
                    || !type.equals(FrameworkConstants.REPOSITORY_LOCAL)) {
                type = application.select(FrameworkConstants.REPOSITORY,
                        String.class);
            }
        }
        ...
        if (cache.exists()) {
            ...
            if (FrameworkConstants.REPOSITORY_LOCAL.equals(type)) {
                String repository = application.select(
                        FrameworkConstants.LOCAL, String.class);
                File local = new File(repository, filename);
                FileUtils.copyFile(local, cache);
                file = cache;
            } else if (FrameworkConstants.REPOSITORY_AMAZON_S3.equals(type)) {
                ...
                bucketName = application.select(
                        FrameworkConstants.AWS_S3_BUCKET_NAME, String.class);
    ...
    @ApiMethod(description = "image download", requestParameters = {
            ...
            @ApiParam(name = "type", type = String.class, allowedvalues = {
                    ...
                    FrameworkConstants.REPOSITORY_AMAZON_S3 }) }, responseObject = byte[].class)
